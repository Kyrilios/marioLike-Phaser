class Scene5 extends Phaser.Scene {
    ...
    preload() {
        console.log("<4th map>")
            // map4 made with Tiled in JSON format
        this.load.tilemapTiledJSON('map4', 'assets/maps/mapB.json');
    ...
    create() {
        ...
        var groundTiles = map4.addTilesetImage('pixelTiles');
        // create the ground layer
        worldLayer = map4.createDynamicLayer('World', groundTiles, 0, 0);
        // the player will collide with this layer
        worldLayer.setCollisionByExclusion([-1]);

        // bg image used as tileset
        var backgroundA = map4.addTilesetImage('pixelTiles');
        // // add BG as tiles
        bgLayer = map4.createDynamicLayer('BG', backgroundA, 0, 0);

        // coins image used as tilesheet
        var coinGold = map4.addTilesetImage('pesoGold');
        // // add coins as tiles
        coinsLayer = map4.createDynamicLayer('Coins', coinGold, 0, 0);

        coinsLayer.setCollisionByExclusion([-1]);

        var coinR = map4.addTilesetImage('pesoRed');
        // // add coins as tiles
        redCoinsLayer = map4.createDynamicLayer('redCoins', coinR, 0, 0);

        redCoinsLayer.setCollisionByExclusion([-1]);



        // var iWall = map4.addTilesetImage('inviWall');
        leftColliderLayer = map4.createDynamicLayer('leftCollider', 0, 0);
        leftColliderLayer.setCollisionByExclusion([-1]);

        //var iWall = map4.addTilesetImage('inviWall');
        rightColliderLayer = map4.createDynamicLayer('rightCollider', 0, 0);
        rightColliderLayer.setCollisionByExclusion([-1]);



        //   var iWall = map4.addTilesetImage('inviWall');
        leftColliderLayer1 = map4.createDynamicLayer('leftCollider1', 0, 0);
        leftColliderLayer1.setCollisionByExclusion([-1]);

        // var iWall = map4.addTilesetImage('inviWall');
        rightColliderLayer1 = map4.createDynamicLayer('rightCollider1', 0, 0);
        rightColliderLayer1.setCollisionByExclusion([-1]);


        // var iWall = map4.addTilesetImage('inviWall');
        leftColliderLayer2 = map4.createDynamicLayer('leftCollider2', 0, 0);
        leftColliderLayer2.setCollisionByExclusion([-1]);

        // var iWall = map4.addTilesetImage('inviWall');
        rightColliderLayer2 = map4.createDynamicLayer('rightCollider2', 0, 0);
        rightColliderLayer2.setCollisionByExclusion([-1]);


        var exit = map4.addTilesetImage('pixelTiles');
        exitLayer = map4.createDynamicLayer('Exit', exit, 0, 0);
        exitLayer.setCollisionByExclusion([-1]);


        var redSpike = map4.addTilesetImage('pixelTiles');
        enemySpikeLayer = map4.createDynamicLayer('enemySpike', redSpike, 0, 0);
        enemySpikeLayer.setCollisionByExclusion([-1]);

        var enemyWalk = map4.addTilesetImage('pixelTiles');
